...
-- SUPERSEDED 2026-08-24: pgvector is not available on this Postgres server
-- (CREATE EXTENSION fails, so every statement below using vector/hnsw/
-- vector_cosine_ops fails right behind it - confirmed live, see
-- iDempiereCLDE 202608241634_CLD-1601.sql for the replacement). That
-- replacement creates AIG_Embedding via the Application Dictionary instead,
-- storing Embedding as a plain VARCHAR (no native vector search) so the
-- table can exist without the extension. Commented out below rather than
-- deleted so the pgvector-native version is easy to restore once the
-- Postgres image actually has the extension.
--
...
-- SUPERSEDED 2026-08-24: AIG_Embedding above is commented out, so there's
-- no such table for this trigger to attach to. The replacement table
-- (202608241634_CLD-1601.sql) doesn't need it either - it's a plain
-- Application Dictionary table, PO.saveEx() already maintains Updated.
--
...
-- SUPERSEDED 2026-08-24: uses vector(768) and the <=> operator, both
-- pgvector-only. AIG_Embedding above is commented out too, so this has
-- nothing to query against either way.
--
